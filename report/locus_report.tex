...
\begin{tikzpicture}
  \def\R{1.8cm}    % main circle radius  (SVG r=15, scaled ×0.12)
  \def\rdot{0.26cm}% center dot radius   (SVG r=3,  scaled ×0.12 × 0.72 ≈ 0.36 → tuned to 0.26)
  \def\tin{1.68cm} % tick inner radius   (SVG dist 14 from centre, scaled)
  \def\tout{2.4cm} % tick outer radius   (SVG dist 20 from centre, scaled)
...
% Wordmark — wide-spaced caps in locusaccent
...
\texttt{gateway}          & 8000 / NodePort 30800 & Public       & API orchestration, auth, rate limiting & EEP \\
\texttt{visual\_engine}   & 8001 & ClusterIP (internal) & CLIP embedding, YOLO detection         & IEP 1 \\
\texttt{attribute\_tagger}& 8004 & ClusterIP (internal) & Gemini VLM attribute extraction        & IEP 2 \\
\texttt{mlops\_exporter}  & 8003 & ClusterIP (internal) & Prometheus ML-pipeline metrics         & Support \\
\texttt{prometheus}       & 9090 & ClusterIP (internal) & Metrics collection                     & Obs. \\
\texttt{grafana}          & 3000 & ClusterIP (internal) & Visualisation dashboards               & Obs. \\
...
% Gateway
\node[gw, below=0.8cm of browser] (gw) {\textbf{Gateway (EEP)}\\[-2pt]\scriptsize FastAPI :8000 / NodePort 30800};

% Visual Engine + Attribute Tagger (side by side)
\node[svc, below left=1.2cm and 1.6cm of gw]  (ve)  {\textbf{Visual Engine (IEP 1)}\\[-2pt]\scriptsize CLIP + YOLO :8001};
\node[svc, below right=1.2cm and 1.6cm of gw] (at)  {\textbf{Attr.\ Tagger (IEP 2)}\\[-2pt]\scriptsize Gemini VLM :8004};
...
\end{tikzpicture}
\caption{Locus runtime architecture. Solid arrows = synchronous HTTP; dashed arrows = Prometheus scrape paths.}
\label{fig:arch}
\end{figure}
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{\texttt{POST /vectorize}}
    \begin{itemize}
      \item \emph{Input:} \texttt{multipart/form-data} with \texttt{file} (pre-cropped image) and optional \texttt{text\_hint} string.
  ...
  \item \textbf{\texttt{POST /index-image}}
    \begin{itemize}
      ...
      \item \emph{Pipeline:} Category voting (title whitelist $\to$ sentence-transformer $\to$ CLIP), 3-tier box selection, crop, embed, upsert to Qdrant.
    \end{itemize}
...
\begin{itemize}[leftmargin=*]
  \item \textbf{\texttt{POST /tag}}
    \begin{itemize}
      ...
      \item \emph{Output:} \texttt{TagResponse \{colors: str[$\le$3], style: str, silhouette: str, occasion: str, pattern: str, material: str, trend\_tags: str[]\}} --- vocabulary-constrained (12 styles, 8 occasions, 10 patterns, 11 material types).
...
\texttt{locus/gateway}          & \texttt{gateway/Dockerfile}          & python:3.11-slim & FastAPI, uvicorn, Qdrant client, PyJWT, bcrypt, slowapi & $\approx$350\,MB \\
\texttt{locus/visual-engine}    & \texttt{visual\_engine/Dockerfile}   & python:3.11-slim & torch 2.6.0+cpu, ultralytics 8.3.2, transformers, PEFT & $\approx$2.1\,GB \\
\texttt{locus/attribute-tagger} & \texttt{attribute\_tagger/Dockerfile} & python:3.11-slim & FastAPI, httpx, Pillow, qdrant-client & $\approx$280\,MB \\
\texttt{locus/mlops-retrain}    & \texttt{mlops/Dockerfile.retrain}    & python:3.11-slim & torch+cpu, transformers, peft, mlflow & $\approx$1.8\,GB \\
...
\begin{tabular}{@{}lllll@{}}
\toprule
\textbf{Deployment} & \textbf{CPU Req/Limit} & \textbf{Mem Req/Limit} & \textbf{Readiness Probe} & \textbf{Initial Delay} \\ \midrule
gateway             & 100m / 500m   & 256\,Mi / 512\,Mi & HTTP GET \texttt{/}        & 10\,s \\
visual-engine       & 500m / 1500m  & 2\,Gi / 3\,Gi     & HTTP GET \texttt{/health}  & 90\,s \\
attribute-tagger    & 100m / 300m   & 192\,Mi / 384\,Mi & HTTP GET \texttt{/health}  & 10\,s \\
...
\begin{lstlisting}[caption={Kubernetes deployment commands on Azure Standard\_B2s}]
...
\begin{longtable}{@{}p{2.8cm}p{2.8cm}p{3.2cm}p{4.2cm}@{}}
\toprule
\textbf{Category} & \textbf{Chosen} & \textbf{Rejected (why not)} & \textbf{Evidence / Measurement} \\ \midrule
\endhead
...
Full fine-tuning --- infeasible on 2-core CPU (\textgreater{}12\,h/epoch, estimated) &
LoRA completes 150 steps in $\approx$35\,min, consumes 1.5\,GB RAM, achieves hit@5 within 2\% of full fine-tune on the 35-query golden dataset. \\[4pt]

Managed vs.\ self-hosted vector DB &
Qdrant Cloud free tier (1\,GB, GCP eu-west3) &
Self-hosted Qdrant in-cluster --- consumes $\approx$512\,MB RAM on the 4\,GB VM &
...
Local LLM (e.g., LLaVA-7B) --- requires $\ge$8\,GB VRAM, exceeds VM memory budget &
Structured extraction (12 styles, 10 patterns, 11 materials) requires a model too large for the 4\,GB VM. Cost is \$2--5/mo at $\approx$10\,K calls/mo---within budget. \\[4pt]
...
NodePort 30800 avoids the Traefik conflict (documented in commit \texttt{eaab19a}) at the cost of a non-standard port. TLS is handled at the ingress layer via cert-manager. \\ \bottomrule
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{LoRA training:} \texttt{mlops/lora\_trainer.py} --- InfoNCE loss, $r=4$, $\alpha=8$, batch 4 with gradient accumulation $\times$4 (effective batch 32), 150 steps ($\approx$35\,min on a 2-core runner).
...
\texttt{tests/test\_vectorizer.py}  & Unit          & $\approx$12 & Category voting, shoe subtypes, CANONICAL\_LABELS, whitelist matching \\
\texttt{tests/test\_smoke.py}       & Smoke         & $\approx$8  & Health probes on all live services; golden dataset label completeness \\
\texttt{tests/test\_pipeline.py}    & Integration   & 50+         & Rate limits, search, detect, feedback, CORS, attribute population \\
\texttt{tests/test\_auth\_store.py} & Integration   & 80+         & Full auth flow, JWT lifecycle, bcrypt, store management, regression cases \\
...
Validate the vectoriser logic in isolation---category voting, bounding box selection tiers, shoe-style classification (\texttt{shoe\_style\_from\_label()}), and whitelist token matching---without requiring any live service. Mocked model outputs ensure determinism.
...
gateway          & \texttt{http\_request\_duration\_seconds} p50/p95 & 4xx/5xx via \texttt{http\_requests\_total} & \texttt{locus\_searches\_total} & \texttt{locus\_rating\_avg\_score} \\
visual\_engine   & \texttt{http\_request\_duration\_seconds} p50/p95 & 5xx rate             & req/s histogram              & \texttt{locus\_clip\_confidence} \\
attribute\_tagger& \texttt{tagger\_latency\_seconds} p50/p95        & \texttt{tagger\_errors\_total} & \texttt{tagger\_requests\_total} & n/a (structured output) \\
...
\texttt{locus\_gemini\_score\_bucket}    & Histogram & Judge score distribution (poor/fair/good/excellent) \\ \bottomrule
\caption{Principal Prometheus metrics exposed by Locus services}
\label{tab:metrics}
\end{longtable}

\paragraph{Grafana.}
A Grafana instance (``Locus --- Admin Dashboard'', UID \texttt{locus-main}, v7) is provisioned via file-based configuration under \texttt{monitoring/grafana/provisioning/}. The Prometheus datasource and dashboard definitions are loaded at container startup; the dashboard auto-refreshes every 15 seconds. The dashboard includes: a Service Health row (up/down status for all four services), retrain progress panels (training loss, judge delta, promotion count), search quality panels (VSS@5 trend, rating average), and system panels (catalog size, broken item count, active sessions).
...
The production system runs on a single \textbf{Azure Standard\_B2s} virtual machine (2 vCPU burstable, 4\,GB RAM, Standard SSD) in the US East region, orchestrated via Kubernetes. All services are deployed as Kubernetes pods managed by the manifests in \texttt{k8s/}.
...
The dominant cost (83\,\%) is the Azure VM. Switching to a 1-year reserved instance reduces this to $\approx$\$20/mo at the cost of upfront commitment. The Qdrant Cloud free tier (1\,GB) accommodates $\approx$100\,K product items at 512 dimensions; the starter paid plan at \$29/mo per 100\,GB covers growth beyond this threshold. CPU-only CLIP inference ($\sim$1--3\,s/query) is acceptable at current traffic volumes; an Azure NC4as T4 v3 GPU spot node (\$0.23/hr) would reduce latency to $<$100\,ms at 10$\times$ current throughput. LLM API costs are bounded by rate limits (30 RPM) and remain below \$50/mo even at 10$\times$ current traffic.
